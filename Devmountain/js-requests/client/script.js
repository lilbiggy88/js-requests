...
/*
    ...
    Use querySelectoutton and save it to a variable called sayHelloButtonr to select that b
*/
...
/*
    ...
    (Why wouldn't I do document.getElementById instead of creating (let repeatEl)?)
...
const getReq = () => {
    axios
    // .get('http://localhost:3000/query-testrepeat?myquery=Hello-There')
    .get('http://localhost:3000/query-testrepeat?myquery=Hello-There&anotherOne=Kenobi')
...
/*
    ...
    1: Send no queries on the URL -- what happened? It said I didn't send a query!

    2: Send more than 1 query on the URL -- what happened? They both popped up!
*/
...
function createFood(body) = => {
    const foodInput = document.querySelector('p')
    const newFood = ``

    axios
    .get('http://localhost:3000/food', + body)
    .then(res => {
        console.log(res.data)
